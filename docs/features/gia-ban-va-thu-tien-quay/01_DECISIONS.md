...
| 4 | **Audit log bất biến** | Bắt buộc. Hai `AuditAction` mới: `DRUG_PRICE_CHANGED`, `SALE_PRICE_OVERRIDE`. ⚠️ `audit_logs.action` là **varchar(32)** (kỷ luật #7 đã ghi sự cố này) — cả hai tên **đo được 19 và 20 ký tự**, nằm trong giới hạn. `drug_price_history` là bảng **chỉ-ghi-thêm**: không `UPDATE`, không `DELETE` |
