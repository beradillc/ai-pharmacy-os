...
async def _seed_catalog(svc: _Services, ctx: RequestContext) -> dict[str, tuple[UUID, int, int]]:
    ...
    # Bản đồ tên hoạt chất -> id. Đọc lại từ CSDL chứ không lấy từ create_ingredient ở trên:
    # lượt đó bọc trong suppress(AppError) nên khi chạy lại seeder nó không trả về gì.
    ing_ids = {i.name: i.id for i in await svc.catalog.list_ingredients(ctx)}

    for index, (name, rx, form, strength, unit, price, velocity) in enumerate(_DRUGS):
        # Hàm lượng để 1 "đơn vị": seeder không giữ hàm lượng thật của từng hoạt chất, và
