...
**2 blocker nền (đã kiểm chứng 2026-07-23):**
...
2. **Văn bản pháp lý thiếu (Bước 1.1/1.8) — ĐÃ BỔ SUNG ĐỦ (2026-07-23, cập nhật đợt 2).** Sếp đã thả đủ: **Luật BVDLCN 91/2025/QH15**, **NĐ 356/2025/NĐ-CP**, **GPP** (TT02/2018 + TT11/2025 + TT29/2020), **Luật Dược 105/2016/QH13** + **Luật sửa đổi 44/2024/QH15**. Đã đọc toàn bộ + tóm tắt vào `docs/legal/*.SUMMARY.md` (10 file, xem `docs/legal/README.md` mục lục). **4/4 văn bản ban đầu yêu cầu nay đã có đủ.** Phát hiện quan trọng: (1) NĐ356 Điều 41.2 — miễn trừ DPIA cho hộ kinh doanh/DN siêu nhỏ KHÔNG áp dụng khi xử lý dữ liệu nhạy cảm (dị ứng/bệnh nền) → mọi tenant dù nhỏ vẫn phải DPIA cho hồ sơ KH; (2) câu hỏi pháp lý mở cần tư vấn luật sư: BeraLLC có cần Giấy chứng nhận kinh doanh dịch vụ xử lý DLCN (NĐ356 Điều 21-27) không; (3) **Luật Dược Điều 2.27-28 + Điều 6.5.h là nguồn Luật (đã tìm thấy) cho rule "ETC cần đơn thuốc"** mà `docs/13_COMPLIANCE_SPEC.md` dòng 14 đang đánh dấu "KHÔNG TÌM THẤY" — cần sếp xác nhận có cập nhật spec đã khóa hay không; (4) **"Chuỗi nhà thuốc"** (Luật 44/2024, khái niệm mới) khớp sẵn với kiến trúc `tenant_id`+`branch_id` hiện tại (`crm.Customer` đã scope theo tenant, không theo branch — đúng hướng), nhưng RBAC/IAM cần role riêng "chuyên môn cấp chuỗi" vs "cấp nhà thuốc" khi thiết kế.
...
> **Trạng thái:** văn bản pháp lý đã đọc + tóm tắt xong, nhưng **chưa quay lại code** hồ sơ KH/tích điểm — blocker RBAC/IAM (mục 1) vẫn còn nguyên, và 2 câu hỏi pháp lý mở ở trên (miễn trừ DPIA, giấy phép xử lý DLCN) cần sếp/luật sư xác nhận trước khi thiết kế chi tiết. Đầu phiên sau: `docker compose ps` xác nhận hạ tầng thật (đừng tin mục "Hạ tầng dev").
