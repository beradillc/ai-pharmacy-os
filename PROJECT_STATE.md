...
| Tối ưu pytest | ✅ **ĐỢT 1 XONG** (§7bl) — **683 s → 296 s** (−57 %); nợ `conftest.py`/alembic đóng luôn cùng đợt. Còn 1 đòn bẩy lớn **chờ Chain quyết**: bcrypt |
...
> **Đọc 1 dòng nếu chỉ đọc 1 dòng:** suite **683 s → 296 s**, 1011 passed, EXIT=0.
...
### 🟡 CÒN LẠI — hai đòn bẩy, CHƯA làm, chờ Chain quyết

Đo lại sau đợt 1 (`tests/integration` = 284,55 s): setup 146,2 s (51,8 %) · call 136,1 s.

| Đòn bẩy | Số đo | Vì sao dừng lại hỏi |
|---|---|---|
| **bcrypt** — `hashpw` 225 lần 65,2 s + `checkpw` 232 lần 67,3 s = **132,5 s = 46,6 % của integration** | rounds=12 → **290 ms/lần**; rounds=4 → **1,2 ms**. Hạ vòng lặp **chỉ trong test** đưa suite về **≈165 s** | Đây là **tham số an toàn**, không phải hằng số hạ tầng. Sửa được bằng monkeypatch test-only, không đụng mã sản phẩm — nhưng nó làm bộ test không còn chạy đúng chi phí băm của production. Kỷ luật #3: quyết định loại này là của Chain |
| **`pytest-xdist`** | RSS đỉnh 1 tiến trình pytest = **139,6 MB**; RAM khả dụng 1,5 GiB ⇒ 4 worker ≈ 560 MB, **thừa sức** | 🔴 Tôi đã **đoán sai** ở đầu phiên rằng RAM sẽ là trần — đo ra thì không phải. Nhưng có vướng thật: `tests/concurrency` dùng **chung một CSDL** và `TRUNCATE` 9 bảng mỗi test ⇒ chạy song song là các worker giẫm lên nhau. Cần CSDL riêng mỗi worker hoặc `--dist loadfile` + tách. Thêm nữa đây là **phụ thuộc mới** trong repo mà CI **chưa chạy lần nào** |
